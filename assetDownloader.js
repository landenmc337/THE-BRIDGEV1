...
async function downloadChannelBadges(
    ...
    console.log(
        `📺 Downloading channel badges for ${username}...`
    );
...
async function downloadBadges(
    ...
    try {
        ...
        console.log(
            "🔍 Checking Twitch badge assets..."
        );

        const token =
            await getAppToken();

        await downloadGlobalBadges(
            token
        );
        ...
        console.log(
            "🎉 All Twitch badges are up to date."
        );
